...
def correct():
  global differences
  global z
  print("You played", note, fixed[" ".join(list(map(str, differences)))]) # Correct Scale (then outputted)
  differences = []
  z = []
...
try:
  while True:
    ...
    if linear_frequency < MAX_FREQUENCY or linear_frequency > MIN_FREQUENCY: # Outside of frequency range (background noise)
      count += 1
      if count <= 25 or len(z) < 1: # If gap not long enough or no scale played yet
        continue
